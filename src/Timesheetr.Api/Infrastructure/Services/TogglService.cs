...
public class TogglService
{
    ...
    private async Task EnsureTagExistsAsync(long workspaceId)
    {
        ...
        await _http.PostAsync($"https://api.track.toggl.com/api/v9/workspaces/{workspaceId}/tags", content);
    }
    ...
    private async Task SetTagAsync(long workspaceId, long entryId, string tag, string action)
    {
        ...
        var response = await _http.PutAsync(
            $"https://api.track.toggl.com/api/v9/workspaces/{workspaceId}/time_entries/{entryId}",
            content);
